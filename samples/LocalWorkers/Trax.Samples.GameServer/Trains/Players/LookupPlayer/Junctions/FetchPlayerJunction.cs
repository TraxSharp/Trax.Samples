using Microsoft.Extensions.Logging;
using Trax.Core.Junction;

namespace Trax.Samples.GameServer.Trains.Players.LookupPlayer.Junctions;

public class FetchPlayerJunction(ILogger<FetchPlayerJunction> logger)
    : Junction<LookupPlayerInput, PlayerProfile>
{
    public override async Task<PlayerProfile> Run(LookupPlayerInput input)
    {
        logger.LogInformation(
            "Looking up player {PlayerId} — fetching profile from database",
            input.PlayerId
        );

        await Task.Delay(50);

        var output = new PlayerProfile
        {
            PlayerId = input.PlayerId,
            Rank = 42,
            Wins = 128,
            Losses = 64,
            Rating = 1847,
        };

        logger.LogInformation(
            "Player {PlayerId} found: rank=#{Rank}, wins={Wins}, losses={Losses}, rating={Rating}",
            output.PlayerId,
            output.Rank,
            output.Wins,
            output.Losses,
            output.Rating
        );

        return output;
    }
}
